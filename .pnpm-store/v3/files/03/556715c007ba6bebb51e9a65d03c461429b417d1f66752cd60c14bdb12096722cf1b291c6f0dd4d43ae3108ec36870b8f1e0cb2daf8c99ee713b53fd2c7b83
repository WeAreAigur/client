                        ...
                        if (ts.isVariableStatement(node) && ((ref = node.modifiers) == null ? void 0 : ref.some((m)=>m.kind === ts.SyntaxKind.ExportKeyword))) {
                            if (ts.isVariableDeclarationList(node.declarationList)) {
                                for (const declaration of node.declarationList.declarations){
                                    if (declaration.getFullStart() <= position && position <= declaration.getFullStart() + declaration.getFullWidth()) {
                                        // `export const ... = ...`
        ...
        proxy.getCompletionsAtPosition = (fileName, position, options)=>{
            ...
            visitEntryConfig(fileName, position, (entryConfig, declaration)=>{
                if (!API_DOCS[entryConfig]) {
                    if (declaration.name.getFullStart() <= position && position <= declaration.name.getFullStart() + declaration.name.getFullWidth()) {
                        prior.entries = [
                            ...prior.entries,
