...
const resolveVerification = (verification)=>{
    const google = resolveAsArrayOrUndefined(verification == null ? void 0 : verification.google);
    const yahoo = resolveAsArrayOrUndefined(verification == null ? void 0 : verification.yahoo);
    let other;
...
const resolveTwitter = (twitter)=>{
    ...
    resolved.images = (ref = resolveAsArrayOrUndefined(twitter.images)) == null ? void 0 : ref.map((item)=>{
...
export async function resolveMetadata(metadataItems) {
...
export async function resolveFileBasedMetadataForLoader(_layer, _dir) {
    let metadataCode = "";
