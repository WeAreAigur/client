...
async function createTreeCodeFromPath({ pagePath , resolve , resolveParallelSegments  }) {
    ...
    async function createSubtreePropsFromSegmentPath(segments) {
        ...
        for (const [parallelKey, parallelSegment] of parallelSegments){
            ...
            if (parallelSegment === PAGE_SEGMENT) {
                ...
                metadataCode += `{
          type: 'page',
          layer: ${// There's an extra virtual segment.
                segments.length - 1},
          mod: () => import(/* webpackMode: "eager" */ ${JSON.stringify(resolvedPagePath)}),
          ...
          page: [() => import(/* webpackMode: "eager" */ ${JSON.stringify(resolvedPagePath)}), ${JSON.stringify(resolvedPagePath)}]}]`;
            ...
            const { treeCode: subtreeCode , treeMetadataCode: subTreeMetadataCode  } = await createSubtreePropsFromSegmentPath([
                ...segments,
                parallelSegment
            ]);
            ...
            const layoutPath = (ref = filePaths.find(([type, path])=>type === "layout" && !!path)) == null ? void 0 : ref[1];
            if (!rootLayout) {
          ...
          path: ${JSON.stringify(layoutPath)},
            ...
            metadataCode += await (0, _resolveMetadata).resolveFileBasedMetadataForLoader(segments.length, await resolve(`${appDirPrefix}${parallelSegmentPath}/`, true));
    ...
    const { treeCode , treeMetadataCode  } = await createSubtreePropsFromSegmentPath([]);
    return {
...
const nextAppLoader = async function nextAppLoader() {
    ...
    const { treeCode , treeMetadataCode , pages: pageListCode , rootLayout , globalError ,  } = await createTreeCodeFromPath({
        pagePath,
        resolve: resolver,
