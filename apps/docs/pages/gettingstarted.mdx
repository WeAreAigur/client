...
We add a new Node using the `flow` builder with the `node` method. The `node` method accepts a Node (which is just a function, more on this later) and a function that will let you configure that Node's input (which is fully typed of course 🎉). We can enter hardcoded values in the input's properties or access previous Nodes or the pipeline's input parameters.

Let's start with a GPT3 Node with hardcoded values.

```ts index.ts focus=3,22:26 mark=3[3:16],24[11:24]
import {
  createClient,
  gpt3Prediction
} from '@aigur/client';
...
### Chain Nodes
Let's add another Node that will ready our prompt for GPT3.
...
```ts index.ts focus=3,17,22:27 mark=3[3:15],17[4:10],24[11:23],25[12:24]
import {
  createClient,
  replaceString
} from '@aigur/client';
...
```ts index.ts focus=29,30 mark=29[30:33],30[14:22]
import {
  createClient,
  replaceString,
  gpt3Prediction
...
```ts index.ts focus=21,32:34 mark=21[4:7],33[6:9]
import {
  createClient,
  replaceString,
  gpt3Prediction
...
```ts index.ts focus=18,21,36:38 mark=18[4:10],21[4:7],37[9:12],38[3:9]
import {
  createClient,
  replaceString,
  gpt3Prediction
