...
Since pipelines can (should) run on the server you can easily invoke them from the client using the `invokeRemote` method.
...
It receives a `url` parameter which is the URL of the server API where the pipeline will run.
