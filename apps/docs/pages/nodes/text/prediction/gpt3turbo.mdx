...
The base `gpt3Prediction` node also supports the `gpt-3.5-turbo` model but only accepts a `prompt` string as input. If you want to send multiple messages to the model, you can use this node.
