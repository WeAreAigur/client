...
Dedicated node to call GPT3.5-Turbo and returns the result as a [Stream](/streaming).
The base `gpt3PredictionStream` node also supports the `gpt-3.5-turbo` model but only accepts a `prompt` string as input. If you want to send multiple messages to the model, you can use this node.
...
```ts index.ts
import { gpt3TurboPredictionStream } from '@aigur/client';

//...
flow.node(gpt3TurboPredictionStream, () => ({
  messages: [
    {
      role: 'user',
